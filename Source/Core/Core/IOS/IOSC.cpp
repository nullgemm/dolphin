                                     ...
                                     const std::vector<u8>& signature, u32 pid) const
{
  ...
  case SUBTYPE_RSA4096:
  {
    ...
    mbedtls_rsa_import_raw(&rsa, entry->data.data(), entry->data.size(), nullptr, 0, nullptr, 0,
                           nullptr, 0, reinterpret_cast<const u8*>(&entry->misc_data), 4);

    int ret = mbedtls_rsa_pkcs1_verify(&rsa, MBEDTLS_MD_SHA1, sha1.size(), sha1.data(), signature.data());
